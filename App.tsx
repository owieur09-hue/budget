...
const App: React.FC = () => {
  ...
  const renderHeader = () => {
    ...
    return (
      <header className="flex-none px-6 pt-10 pb-6 bg-black text-white flex justify-between items-end rounded-b-[2.5rem] shadow-xl min-h-[130px]">
        <div className="flex flex-col gap-1">
          <span className="text-xs text-gray-400 font-round">{getYear(currentDate)}</span>
          <div className="flex items-center gap-4 mt-1">
             <button onClick={handlePrevMonth} className="text-2xl font-round p-1">{'<'}</button>
             <h1 className="text-3xl font-bold font-round">{getMonth(currentDate) + 1}월</h1>
             <button onClick={handleNextMonth} className="text-2xl font-round p-1">{'>'}</button>
          </div>
        </div>
        <div className="text-right pb-1">
          <p className="text-[10px] text-gray-400 font-round">총 잔액</p>
          <p className="text-xl font-bold font-round">{getMonthBalance(currentDate).toLocaleString()} <span className="text-xs font-normal opacity-50">KRW</span></p>
        </div>
      </header>
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-screen w-full max-w-md mx-auto bg-white relative shadow-2xl overflow-x-hidden">
      {renderHeader()}
      ...
      {!activeSettingsTab && !isDetailsOpen && (
        <button 
          ...
          className="fixed bottom-28 right-6 w-14 h-14 bg-black rounded-full flex items-center justify-center text-white shadow-2xl z-40 active:scale-95 transition-transform"
        >
          <Plus size={28} />
        </button>
      )}

      {/* 하단 바: 페이지의 마지막에 위치 (콘텐츠를 가리지 않음) */}
