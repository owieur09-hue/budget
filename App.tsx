      ...
      <header className="flex-none px-6 pt-8 pb-6 bg-black text-white flex justify-between items-end z-10 rounded-b-[2.5rem] shadow-xl min-h-[120px] box-border">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-4 mt-1">
             <button onClick={handlePrevMonth} className="text-2xl font-round">{'<'}</button>
             <h1 className="text-3xl font-round font-bold">{getMonth(currentDate) + 1}월</h1>
             <button onClick={handleNextMonth} className="text-2xl font-round">{'>'}</button>
          </div>
        </div>
        <div className="text-right pb-1">
          <p className="text-[10px] text-gray-400 mb-0.5 opacity-80">총 잔액</p>
          <p className="text-xl font-bold">{getMonthBalance(currentDate).toLocaleString()} <span className="text-xs font-normal text-gray-500">KRW</span></p>
        </div>
      </header>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white sm:rounded-[2rem] sm:border-8 sm:border-gray-200 overflow-hidden relative shadow-2xl">
      ...
      {!activeSettingsTab && !isDetailsOpen && (
        <button onClick={() => setIsTransactionFormOpen(true)} className="absolute bottom-24 right-6 w-14 h-14 bg-black rounded-full flex items-center justify-center text-white shadow-xl z-20"><Plus size={28} /></button>
      )}
