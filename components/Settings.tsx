...
import { X, Plus, Trash2, Edit2, Upload, Save, ChevronDown, ChevronUp } from 'lucide-react';
...
const Settings: React.FC<SettingsProps> = ({
  ...
  const handleAddProfile = () => {
    ...
    setProfiles(prev => [...prev, {
      ...
      color: 'bg-black', // Default to black
            ...
            <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="flex gap-2">
                 <select 
                   ...
                   className="bg-white border border-gray-200 rounded-xl text-xs px-2 font-bold text-black outline-none h-11"
                 >
                 ...
                 <input 
                  ...
                  placeholder="이모지 + 이름"
                  className="flex-1 bg-white border border-gray-200 rounded-xl px-4 text-sm outline-none focus:border-black h-11"
                 />
                 <button onClick={handleAddCategory} className="bg-black text-white px-4 rounded-xl hover:bg-gray-800 h-11 shadow-md">
                   <Plus size={20} />
                 </button>
              </div>
            </div>
              ...
              <div className="space-y-3">
                 <div className="flex gap-2">
                    <select 
                      ...
                      className="bg-white border border-gray-200 rounded-xl text-xs px-2 font-bold text-black outline-none h-10"
                    >
                      <option value="EXPENSE">지출</option>
                      <option value="INCOME">수입</option>
                    </select>
                    ...
                    <select 
                      ...
                      className="flex-1 bg-white border border-gray-200 rounded-xl text-xs px-2 text-black outline-none h-10"
                    >
                 ...
                 return (
                   <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center shadow-sm">
                      <div>
                        ...
                        <div className="text-xs text-gray-500 pl-1">{item.memo}</div>
                      </div>
            ...
            <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="flex gap-2 items-center">
                 <button 
                   ...
                   className="w-11 h-11 bg-white flex items-center justify-center text-gray-400 rounded-full border border-gray-200 hover:border-black overflow-hidden relative shrink-0 shadow-sm"
                 >
                   {newProfileImage ? (
                 ...
                 <input 
                  ...
                  className="flex-1 bg-white border border-gray-200 rounded-xl px-4 text-sm outline-none focus:border-black h-11"
                 />
                 <button onClick={handleAddProfile} className="bg-black text-white px-4 rounded-xl hover:bg-gray-800 h-11 shadow-md">
                   <Plus size={20} />
                 </button>
              </div>
            </div>
                    ...
                    {editingProfileId === p.id ? (
                      <div className="flex items-center gap-3">
                        <button 
                           ...
                           className="w-10 h-10 bg-white flex items-center justify-center text-gray-400 rounded-full border border-gray-200 overflow-hidden"
                         >
                           {editImage ? (
                        ...
                        />
                        <button onClick={saveEditing} className="text-black bg-gray-100 rounded-lg p-2">
                          <Save size={18} />
                        </button>
                      </div>
                    ) : (
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center text-white overflow-hidden shadow-sm">
                             {p.image ? (
                               ...
                               <span className="font-bold text-sm font-round">{p.name.substring(0, 1)}</span>
                             )}
                          </div>
                          <span className="font-bold text-gray-800 text-sm">{p.name}</span>
                        </div>
                        <div className="flex gap-2">
                           <button onClick={() => startEditing(p)} className="text-gray-300 hover:text-black p-2">
                             <Edit2 size={16} />
                           </button>
                           {profiles.length > 1 && (
                            <button onClick={() => handleDeleteProfile(p.id)} className="text-gray-300 hover:text-red-500 p-2">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
