...
const Settings: React.FC<SettingsProps> = ({
                ...
                {categories.filter(c => c.type === type).map(c => {
                    ...
                    return (
                        <div key={c.id} className="relative group flex flex-col items-center p-1">
                             ...
                             <div className="w-14 h-14 rounded-[1.2rem] bg-gray-50 border border-gray-100 flex items-center justify-center text-2xl shadow-sm mb-2">
                                {icon}
                             </div>
  ...
  return (
    /* h-full과 bg-white를 통해 바닥까지 흰 배경을 강제합니다 */
    <div className="flex flex-col h-full bg-white animate-in slide-in-from-bottom-5 duration-300 overflow-hidden relative">
      ...
      {/* Content 영역: 달력 탭과 동일한 스크롤 및 여백 전략 */}
             ...
             <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="space-y-3">
                 <div className="flex gap-2 w-full">
                    <select value={fixedType} onChange={(e) => setFixedType(e.target.value as TransactionType)} className="flex-none bg-white border border-gray-200 rounded-xl text-xs px-2 font-bold h-10 outline-none">
                      <option value="EXPENSE">지출</option>
                      <option value="INCOME">수입</option>
                    </select>
                    <select value={fixedDay} onChange={(e) => setFixedDay(parseInt(e.target.value))} className="flex-none bg-white border border-gray-200 rounded-xl text-xs px-2 font-bold h-10 outline-none">
                      {Array.from({length: 31}, (_, i) => i + 1).map(d => <option key={d} value={d}>{d}일</option>)}
                      <option value={32}>말일</option>
                    </select>
                    <select value={fixedCatId} onChange={e => setFixedCatId(e.target.value)} className="flex-1 bg-white border border-gray-200 rounded-xl text-xs px-2 outline-none h-10">
                 ...
                 <input type="text" value={fixedMemo} onChange={e => setFixedMemo(e.target.value)} placeholder="메모" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none" />
                 <button onClick={handleAddFixed} className="w-full bg-black text-white py-3.5 rounded-xl font-bold shadow-md">추가하기</button>
              </div>
            </div>
            <div className="space-y-3">
              {fixedItems.map(item => (
                <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center shadow-sm">
                   ...
                   <span className="font-mono font-bold text-sm">{item.amount.toLocaleString()}원</span>
                </div>
              ))}
            </div>
          </div>
               ...
               {profiles.map(p => (
                 <div key={p.id} className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center text-white overflow-hidden shadow-sm">
