...
const Settings: React.FC<SettingsProps> = ({
        ...
        {isOpen && (
            <div className="grid grid-cols-4 gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
                {categories.filter(c => c.type === type).map(c => {
                    ...
                    return (
                        <div key={c.id} className="relative group flex flex-col items-center p-1 cursor-pointer" onClick={() => startEditingCat(c)}>
                             {!c.isDefault && (
                                <button onClick={(e) => { e.stopPropagation(); handleDeleteCategory(c.id); }}
                                  className="absolute -top-1 -right-1 z-10 bg-white text-gray-300 border border-gray-100 rounded-full p-1 shadow-sm hover:text-red-500"
                                >
                                    <X size={12} strokeWidth={3} />
                                </button>
                             )}
                             <div className="w-14 h-14 rounded-[1.2rem] bg-gray-50 border border-gray-100 flex items-center justify-center text-2xl shadow-sm mb-2 hover:border-black transition-colors">
                                {icon}
                             </div>
  ...
  return (
    <div className="flex flex-col h-full bg-white animate-in slide-in-from-bottom-5 duration-300 overflow-hidden">
      {/* 고정 헤더 */}
      <div className="pt-12 pb-2 px-6 flex-none bg-white z-20">
        <div className="relative flex items-center justify-center mb-8">
            <div className="absolute inset-0 flex items-center"><div className="w-full h-0.5 bg-black rounded-full"></div></div>
            <div className="relative bg-black text-white px-6 py-2 rounded-full text-lg font-bold font-round z-10 shadow-lg">{getTitle()}</div>
        </div>
      </div>
      
      {/* 스크롤 영역: pb-32로 하단바 공간 확보 및 배경 유지 */}
          ...
          <div className="space-y-8">
            <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="flex gap-2">
                 ...
                 <input type="text" value={newCatName} onChange={e => setNewCatName(e.target.value)} placeholder="이모지 + 이름" className="flex-1 bg-white border border-gray-200 rounded-xl px-4 text-sm h-11 outline-none" />
          ...
          <div className="space-y-8">
             <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="space-y-3">
                 <div className="flex gap-2">
                    <select value={fixedType} onChange={(e) => setFixedType(e.target.value as TransactionType)} className="bg-white border border-gray-200 rounded-xl text-xs px-2 font-bold h-10 outline-none">
                      <option value="EXPENSE">지출</option>
                      <option value="INCOME">수입</option>
                    </select>
                    ...
                    <select value={fixedCatId} onChange={e => setFixedCatId(e.target.value)} className="flex-1 bg-white border border-gray-200 rounded-xl text-xs px-2 outline-none">
                      <option value="">카테고리 선택</option>
                      {categories.filter(c => c.type === fixedType).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                 </div>
                 <input type="text" value={fixedAmount} onChange={e => setFixedAmount(e.target.value.replace(/[^0-9]/g, ''))} placeholder="금액" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-right outline-none" />
                 <button onClick={handleAddFixed} className="w-full bg-black text-white py-3.5 rounded-xl font-bold">추가하기</button>
              </div>
            </div>
            <div className="space-y-3">
              {fixedItems.map(item => (
                <div key={item.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center shadow-sm">
                   <span className="text-sm font-bold">{categories.find(c => c.id === item.categoryId)?.name}</span>
                   <span className="font-mono font-bold">{item.amount.toLocaleString()}원</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
              ...
              <div className="flex gap-2 items-center">
                 <button onClick={() => newProfileFileInputRef.current?.click()} className="w-11 h-11 bg-white rounded-full border border-gray-200 overflow-hidden flex items-center justify-center">
                   {newProfileImage ? <img src={newProfileImage} className="w-full h-full object-cover" /> : <Upload size={18} className="text-gray-400" />}
                 </button>
